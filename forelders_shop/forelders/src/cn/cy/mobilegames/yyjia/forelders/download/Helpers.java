...
public class Helpers {
	...
	public static String generateSaveFile(Context context, String url,
		...
		if (destination == Impl.DESTINATION_FILE_URI) {
	...
	private static String getPathForFileUri(String url, String hint,
		...
		if (!isExternalMediaMounted()) {
			throw new GenerateSaveFileError(Impl.STATUS_DEVICE_NOT_FOUND_ERROR,
		...
		if (path.endsWith("/")) {
			...
			throw new GenerateSaveFileError(
					Impl.STATUS_FILE_ALREADY_EXISTS_ERROR,
	...
	private static void checkCanHandleDownload(Context context,
		...
		if (destination == Impl.DESTINATION_EXTERNAL) {
			if (mimeType == null) {
				throw new GenerateSaveFileError(Impl.STATUS_NOT_ACCEPTABLE,
	...
	private static File getExternalDestination(long contentLength)
		...
		if (!base.isDirectory() && !base.mkdir()) {
			...
			// read-only.
			throw new GenerateSaveFileError(Impl.STATUS_FILE_ERROR,
	...
	/* package */static void deleteFile(ContentResolver resolver, long id,
		...
		resolver.delete(Impl.ALL_DOWNLOADS_CONTENT_URI, BaseColumns._ID + " = ? ",
