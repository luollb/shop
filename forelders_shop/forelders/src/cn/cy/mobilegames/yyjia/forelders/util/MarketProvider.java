...
import cn.cy.mobilegames.yyjia.forelders.download.DownloadProvider;
...
public class MarketProvider extends ContentProvider {
	...
	public static final String AUTHORITY = "cn.cy.mobilegames.yyjia.forelders";
	...
	static {
		...
		sURIMatcher.addURI(AUTHORITY, TABLE_SEARCH_HISTORY_ID,
				SEARCH_HISTORY_ID);
		sURIMatcher.addURI(AUTHORITY, TABLE_PRODUCTS, PRODUCTS);
		sURIMatcher.addURI(AUTHORITY, TABLE_UPGRADES, UPGRADE_PRODUCT);
		sURIMatcher.addURI(AUTHORITY, TABLE_BUY, BUY_PRODUCT);
		sURIMatcher.addURI(AUTHORITY, TABLE_CARD, CARD);
	}

	/**
	 * The content:// URI to access search history
	 */
	public static final Uri SEARCH_CONTENT_URI = Uri.parse(jointUri(AUTHORITY,
			TABLE_SEARCH_HISTORY));

	/**
	 * return "content://" + auth + "/" + path
	 * 
	 * @param auth
	 * @param path
	 * @return
	 */
	private static String jointUri(String auth, String path) {
		return "content://" + auth + "/" + path;
	}
	 ...
	 */
	public static final Uri PRODUCTS_CONTENT_URI = Uri.parse(jointUri(
			AUTHORITY, TABLE_PRODUCTS));

	/**
	 * The content:// URI to access upgrade products
	 */
	public static final Uri UPGRADE_CONTENT_URI = Uri.parse(jointUri(AUTHORITY,
			TABLE_UPGRADES));

	/**
	 * The content:// URI to access purchased products
	 */
	public static final Uri BUY_CONTENT_URI = Uri.parse(jointUri(AUTHORITY,
			TABLE_BUY));

	/**
	 * The content:// URI to access card
	 */
	public static final Uri CARD_CONTENT_URI = Uri.parse(jointUri(AUTHORITY,
			TABLE_CARD));
	...
	private final class DatabaseHelper extends SQLiteOpenHelper {
		...
		private void createSearchHistoryTable(SQLiteDatabase db) {
			try {
				...
				Utils.D("couldn't create " + TABLE_SEARCH_HISTORY
						+ " table in market database");
				throw ex;
			}
		}
		...
		private void createProductTable(SQLiteDatabase db) {
			try {
				...
				db.execSQL("CREATE TABLE " + TABLE_PRODUCTS + "(" + COLUMN_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + COLUMN_P_ID
						+ " TEXT, " + COLUMN_P_NAME + " TEXT, "
						+ COLUMN_P_PACKAGE_NAME + " TEXT, " + COLUMN_P_CATEGORY
						+ " TEXT, " + COLUMN_P_PAYMENT_TYPE + " TEXT, "
						+ COLUMN_P_SIZE + " TEXT, " + COLUMN_P_PRICE
						+ " TEXT, " + COLUMN_P_ICON_URL + " TEXT, "
						+ COLUMN_P_IS_RECOMMEND + " TEXT);");
			} catch (SQLException ex) {
				Utils.D("couldn't create " + TABLE_PRODUCTS
						+ " table in market database");
				throw ex;
			}
		}
		...
		private void createUpgradeTable(SQLiteDatabase db) {
			try {
				...
				db.execSQL("CREATE TABLE " + TABLE_UPGRADES + "(" + COLUMN_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + COLUMN_P_ID
						+ " TEXT, " + COLUMN_P_PACKAGE_NAME + " TEXT, "
						+ COLUMN_P_NEW_VERSION_NAME + " TEXT, "
						+ COLUMN_P_NEW_VERSION_CODE + " TEXT, "
						+ COLUMN_P_DOWN_URL + " TEXT, " + COLUMN_P_ICON_URL
						+ " TEXT, " + COLUMN_P_NAME + " TEXT, " + COLUMN_P_SIZE
						+ " TEXT, " + COLUMN_P_UPGRADE_INFO + " TEXT, "
						+ COLUMN_P_IGNORE + " INTEGER);");
			} catch (SQLException ex) {
				Utils.D("couldn't create " + TABLE_UPGRADES
						+ " table in market database");
				throw ex;
			}
		}
		...
		private void createPurchesdTable(SQLiteDatabase db) {
			try {
				...
				db.execSQL("CREATE TABLE " + TABLE_BUY + " (" + COLUMN_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT ," + COLUMN_P_ID
						+ " TEXT ," + COLUMN_P_PACKAGE_NAME + " TEXT);");
			} catch (SQLException ex) {
				Utils.D("couldn't create " + TABLE_BUY
						+ " table in market database");
				throw ex;
			}
		}
		...
		private void createCardTable(SQLiteDatabase db) {
			try {
				...
				db.execSQL("CREATE TABLE " + TABLE_CARD + " (" + COLUMN_ID
						+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
						+ COLUMN_CARD_NAME + " TEXT," + COLUMN_CARD_PAY_TYPE
						+ " TEXT," + COLUMN_CARD_ACCOUNTNUM + " INTEGER,"
						+ COLUMN_CARD_PASSWORDNUM + " INTEGER,"
						+ COLUMN_CARD_CREDIT + " TEXT);");
			} catch (SQLException ex) {
				Utils.D("couldn't create " + TABLE_CARD
						+ " table in market database");
				throw ex;
			}
		}
	...
	public String getType(Uri uri) {
		...
		case UPGRADE_PRODUCT:
			...
			return LIST_TYPE + TABLE_UPGRADES;
	 ...
	 * 打印 【查询SQL】 详细信息
	 */
	private static void logVerboseQueryInfo(String[] projection,
