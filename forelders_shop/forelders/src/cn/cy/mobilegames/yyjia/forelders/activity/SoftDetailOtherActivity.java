 ...
 * 应用详情
 */
public class SoftDetailOtherActivity extends BaseActivity implements Observer {
	...
	// private Softinfo soft;
	// private SubSoftinfo subSoft;
	// private CateItem cateItemList;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			if (data != null) {
				...
				// packageName = mDownloadManager.queryPackageName(Integer
				// .valueOf(appid));
			}
	...
	private void initDownTask(Softinfo softInfo, String packageName) {
		if (softInfo != null) {
			if (TextUtils.isEmpty(softInfo.getDownurl())) {
				ToastUtil.showShortToast(context, "下载地址无效,请检查.");
				return;
			}
			Uri srcUri = Uri.parse(softInfo.getDownurl());
			String package_name = packageName == null ? softInfo.getSourceurl()
					: packageName;
			...
			request.setDescription(softInfo.getSummary());
	...
	private void initAppInfo(DownloadInfo info) {
		if (isViewFinish) {
			Utils.D(" info mstatus " + info.mStatus);
			switch (info.mStatus) {
			// STATUS_SUCCESS
			case DownloadManager.STATUS_SUCCESS:
