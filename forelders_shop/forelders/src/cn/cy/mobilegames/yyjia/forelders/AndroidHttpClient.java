...
public final class AndroidHttpClient implements HttpClient {
	...
	/**
	 * Indicate if the HTTP client loaded user's own cookies(such as login
	 * session)
	 * 
	...
	public void addRequestInterceptor(HttpRequestInterceptor interceptor) {
		...
		this.delegate.addRequestInterceptor(interceptor,
				this.delegate.getRequestInterceptorCount());
	}
	...
	public void addResponseInterceptor(HttpResponseInterceptor interceptor) {
		...
		this.delegate.addResponseInterceptor(interceptor,
				this.delegate.getResponseInterceptorCount());
	}
	...
	/**
	 * Modifies a request to indicate to the server that we would like a kind of
	 * MIME type response.(Uses the "Content-Type" HTTP header.)
	 * 
	 * @param request
	 *            the request to modify
	 * @param contentType
	 *            MEME type
	 */
	public static void modifyRequestContentType(HttpRequest request,
			String contentType) {
	...
	/**
	 * Use cmwap gateway(10.0.0.172:80) as proxy for the normal internet
	 * connection
	 */
	public void useProxyConnection(HttpHost proxy) {
		// HttpHost proxy = new HttpHost(host, port, "http");
		this.delegate.getParams().setParameter(ConnRoutePNames.DEFAULT_PROXY,
				proxy);
	}
