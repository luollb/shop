...
class DownloadNotification {
	...
	static final String WHERE_RUNNING = "(" + Impl.COLUMN_STATUS
			+ " >= '100') AND (" + Impl.COLUMN_STATUS + " <= '199') AND ("
			+ Impl.COLUMN_VISIBILITY + " IS NULL OR " + Impl.COLUMN_VISIBILITY
			+ " == '" + Impl.VISIBILITY_VISIBLE + "' OR "
			+ Impl.COLUMN_VISIBILITY + " == '"
			+ Impl.VISIBILITY_VISIBLE_NOTIFY_COMPLETED + "')";
	static final String WHERE_COMPLETED = Impl.COLUMN_STATUS + " >= '200' AND "
			+ Impl.COLUMN_VISIBILITY + " == '"
			+ Impl.VISIBILITY_VISIBLE_NOTIFY_COMPLETED + "'";
	...
	private void updateActiveNotification(Collection<DownloadInfo> downloads) {
		...
		for (DownloadInfo download : downloads) {
			...
			if (download.mStatus == Impl.STATUS_QUEUED_FOR_WIFI
		...
		for (NotificationItem item : mNotifications.values()) {
			...
			intent.setData(ContentUris.withAppendedId(
					Impl.ALL_DOWNLOADS_CONTENT_URI, item.mId));
	...
	private void updateCompletedNotification(Collection<DownloadInfo> downloads) {
		for (DownloadInfo download : downloads) {
			...
			// Add the notifications
			Notification n = TaskUtil.getNotification();
			...
			Uri contentUri = ContentUris.withAppendedId(
					Impl.ALL_DOWNLOADS_CONTENT_URI, id);
			String caption;
			Intent intent;
			if (Impl.isStatusError(download.mStatus)) {
				...
				if (download.mDestination == Impl.DESTINATION_EXTERNAL) {
...
//					PendingIntent.getBroadcast(mContext, 0, intent, 0));
	...
	private boolean isActiveAndVisible(DownloadInfo download) {
		return 100 <= download.mStatus && download.mStatus < 200
				&& download.mVisibility != Impl.VISIBILITY_HIDDEN;
	}

	private boolean isCompleteAndVisible(DownloadInfo download) {
		return download.mStatus >= 200
				&& download.mVisibility == Impl.VISIBILITY_VISIBLE_NOTIFY_COMPLETED;
	}
