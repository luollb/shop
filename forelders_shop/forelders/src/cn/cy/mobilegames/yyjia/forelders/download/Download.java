...
public class Download implements Serializable {
	...
	public Download(int downloadId, String url, String localPath) {
		...
		Log.d("Download", "下载地址；" + url);

		mDownloadId = downloadId;
		mUrl = url;
		String[] tempArray = url.split("/");
		mFileName = tempArray[tempArray.length - 1] + downloadId;
		...
		try {
			...
			if (httpCode >= 200 && httpCode <= 300) {
				...
				while (-1 != (len = in.read(bytes))) {
					...
					// Log.d("Download",(int)(downloadedLength/(float)remoteFileLength
					// *
					// 100));
					if ((int) (downloadedLength / (float) remoteFileLength * 100) % 10 == 0) {
						...
						handler.sendMessage(msg);
						// Log.d("Download",mDownloadId + "已下载" +
						// downloadedLength);
					}
